...
int toneindex = 16; //Index into tone arrays, depending upon key pressed. I nitialize to invalid key for proper LCD display on start.
...
//                          2-10µF
  ...
  if (volume_atten > 238){  //Check if EE memory is out of allowable volume attenuation range
    volume_atten = 238; //Reset to no attenuation and save, if it is  
    ...
    case PRESSED:
           switch (key) {
            case '1':
              ...
              //If not in playback and tone mode is MF, play tone at current duration and update keybuffer
              else if (mfmode == dtmf && playbackmode == off) {playDTMF(toneindex,delayMF); updatebuffer();} 
              //If not in playback and tone mode is DTMF, play tone at current duration and update keybuffer
              else if (mfmode == c5 && playbackmode == off) {playMF(toneindex,delayMF); updatebuffer();} 
              //If not in playback and tone mode is C5, play tone at current duration and update keybuffer
              else if (mfmode == redbox && playbackmode == off) {redBox(1,66,1); updatebuffer();} 
              //If in redbox mode, play nickel coin tone once
              ...
              //If not in playback and tone mode is MFR2, play tone at current duration and update keybuffer
           ...
           case '9':
              ...
              else if (mfmode == redbox && playbackmode == off) {playMF(11,700); updatebuffer();} 
              //Combined operator Coin Collect and Operator Released Green Box tone (MF "ST")
              else if (mfmode == ss4 && playbackmode == off) {ss4Signal(toneindex); updatebuffer();}
              else if (mfmode == dialpulse && playbackmode == off) {pulse(toneindex); updatebuffer();}
              else if (mfmode == mfr2 && playbackmode == off) {playMFR2(toneindex,delayMF); updatebuffer();}
              else if (mfmode == ac1 && playbackmode == off) {pulseAC1(toneindex); updatebuffer();}
    ...
    case HOLD:
        switch (key) {
            ...
            case '3':
                ...
                else if(recordmode == off && playbackmode == off) {mfmode = c5; playmodechange(); EEPROM.write(mfmodeEE, mfmode);} 
                //Change to C5 mode if not in record or playback modes
              break;
            case '4':
                if(recordmode == on) {memrecordend(EEmemory4);}
                else if(recordmode == off && playbackmode == off) {mfmode = ss4; playmodechange(); EEPROM.write(mfmodeEE, mfmode);} 
                //Change to SS4 mode if not in record or playback modes
              break;
            case '5':
                if(recordmode == on) {memrecordend(EEmemory5);}
                else if(recordmode == off && playbackmode == off) {mfmode = dialpulse; playmodechange(); EEPROM.write(mfmodeEE, mfmode);} 
                //Change to Dial Pulse mode if not in record or playback modes
              break;
            case '6':
                if(recordmode == on) {memrecordend(EEmemory6);}
                else if(recordmode == off && playbackmode == off) {mfmode = redbox; playmodechange(); EEPROM.write(mfmodeEE, mfmode);} 
                //Change to Red Box mode if not in record or playback modes
              break;
            case '7':
                if(recordmode == on) {memrecordend(EEmemory7);}
                else if(recordmode == off && playbackmode == off) {mfmode = mfr2; playmodechange(); EEPROM.write(mfmodeEE, mfmode);} 
                //Change to MFR2 mode if not in record or playback modes
              break;
            case '8':
                if(recordmode == on) {memrecordend(EEmemory8);}
                else if(recordmode == off && playbackmode == off) {mfmode = ac1; playmodechange(); EEPROM.write(mfmodeEE, mfmode);} 
                //Change to AC1 mode if not in record or playback modes
              break;
            case '9':
                if(recordmode == on) {memrecordend(EEmemory9);}
                else if(recordmode == off && playbackmode == off) {mfmode = ac9; playmodechange(); EEPROM.write(mfmodeEE, mfmode);} 
                //Change to AC9 mode if not in record or playback modes
              break;
            case '*':
                if(recordmode == on) {memrecordend(EEmemory10);}
                else if(recordmode == off && playbackmode == off) {mfmode = imtsani; playmodechange(); EEPROM.write(mfmodeEE, mfmode);} 
                //Change to IMTS ANI mode if not in record or playback modes
              break;
            case '0':
                if(recordmode == on) {memrecordend(EEmemory11);}
                else if(recordmode == off && playbackmode == off) {mfmode = mtsdial; playmodechange(); EEPROM.write(mfmodeEE, mfmode);} 
                //Change to MTS Dial Pulse mode if not in record or playback modes
              break;
            case '#':
                if(recordmode == on) {memrecordend(EEmemory12);}
                else if(recordmode == off && playbackmode == off) {mfmode = imtsdial; playmodechange(); EEPROM.write(mfmodeEE, mfmode);} 
                //Change to IMTS Dial Pulse mode if not in record or playback modes  
              break;
            case 'A':
            ...
            case 'B':
                ...
                else if(recordmode == on) {memrecordend(abortrecord); updateDisplay();} 
                //If in record mode, abort record mode without writing EEPROM on long "B" press
              break;
            case 'C':
...
void imtsANI(int signal)
{
    ...
    for(int i=1; i<=signal; i++){
      ...
      if (digitalRead(ledPin) == HIGH) {digitalWrite(ledPin, LOW);} else{digitalWrite(ledPin, HIGH);}
      if (i % 2) {delay(25);} //If pulse is odd, just delay with no tone, to generate parity indication
      else{playPWM(2150, 0, 25);} //If pulse is even, play guard tone, to generate parity indication
      if(i == signal){
        if((i % 2)) {delay(190);} //If last pulse is odd, just delay inter-digit time
...
void playPWM(float freq1, float freq2, unsigned long length)
{
  ...
  notePlayer.trigger(0); //Start tone 1
  notePlayer.trigger(1); //Start tone 2 
  ...
  notePlayer.trigger(0); // Turn off tone 1
  notePlayer.trigger(1); 
